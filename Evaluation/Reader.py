...
# Initiate reader and define the main parameters for the data reader
    def __init__(self, ImageDir,MaskDirs,NumClasses,ClassBalance=True, MinPrecision=0.0,MaxBatchSize=100,MinSize=250,MaxSize=900,MaxPixels=800*800*5,TrainingMode=True,AugmentImage=False):
        print("Start")
...
# ----------------------------------------Create reader for data set--------------------------------------------------------------------------------------------------------------
        ...
        for ii,MaskDir in enumerate(MaskDirs):
            uu=0
            print("Prepering files for reader this might take a while")
...
# Crop and resize image and mask and ROI to feet batch size
    def CropResize(self,Img, PredMask,Hb,Wb):
        # ========================resize image if it too small to the batch size==================================================================================
        if PredMask.sum()==0:
            print("545454")
        bbox= cv2.boundingRect(((PredMask)>0).astype(np.uint8))
        [h, w, d] = Img.shape
        Rs = np.max((Hb / h, Wb / w))
...
#-------------------------Augment-----------------------------------------------------------------------------------------------
                if self.AugmentImage: Img,MaskGT,MaskPred=self.Augment(Img,Mask,np.min([float(1000/CatSize)*0.27+0.03,1]))
#-----------------------------------------------------------------------------------------------------------------------------------------
...
# For training
            ...
            IOU=self.BIOU
            self.StartLoadBatch()
            return Imgs, Mask,IOU
########################################################################################################################################################################################
#Load single with no augmentation
    def LoadSingle(self,ClassBalance=True):
            # -----------------------------------Image and resize-----------------------------------------------------------------------------------------------------
           ...
           # print(Ann["GTMaskFile"])
            ...
            if (Img.ndim == 2):  # If grayscale turn to rgb
                Img = np.expand_dims(Img, 3)
                Img = np.concatenate([Img, Img, Img], axis=2)
            IOU = Img[:, :, 0:3]  # Get first 3 channels incase there are more
            # -----------------------------------Crop and resize-----------------------------------------------------------------------------------------------------
            IOU = Ann["IOU"]
            Mask = cv2.imread(Ann["MaskFile"], 0)  # Load segment to refine
            Mask = cv2.resize(Mask, (Img.shape[1], Img.shape[0]), interpolation=cv2.INTER_NEAREST)
...
############################################################################################################################################
#Get class name  information for specific catagory/Class id
