...
########################################################################################################################
    def __init__(self, NumClasses=2):
        ...
        # --------------Build layers for standart FCN with only image as input------------------------------------------------------
            ...
            #-----------------Attention Layer for input segment----------------------------------------------------------------------------------------
            self.InMaskEncoder = nn.Conv2d(1, 64, stride=2, kernel_size=3, padding=1, bias=True)
...
##########################################################################################################################################################
    def forward(self,Images,InMask,UseGPU=True,TrainMode=True):

#----------------------Convert image to pytorch and normalize values-----------------------------------------------------------------
                RGBMean = [123.68,116.779,103.939]
                RGBStd = [65,65,65]
                if TrainMode: tp=torch.FloatTensor
                else:
                    self.half()
                    tp=torch.HalfTensor
                    self.eval()
                ...
                InMask = torch.autograd.Variable(torch.from_numpy(InMask.astype(np.float)), requires_grad=False).unsqueeze(dim=1).type(tp)
