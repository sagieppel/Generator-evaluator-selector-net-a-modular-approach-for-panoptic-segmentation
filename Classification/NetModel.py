...
#------------------------Attention roi  proccesssing layer---------------------------------------------------------------------------------------
            self.AttentionROI = nn.Conv2d(1, 64, stride=2, kernel_size=3, padding=1,bias=True)  # Generate attention filters their output will be multiplied with the main net feature map
...
#------------------------------- Convert from numpy to pytorch-------------------------------------------------------
                InpImages = torch.autograd.Variable(torch.from_numpy(Images.astype(np.float)), requires_grad=False).transpose(2,3).transpose(1, 2).type(tp)
...
#
                ...
                AttentionMap = self.AttentionROI(ROImap)
                x = x + AttentionMap
                # print("Attention map sum")
                # print(self.AttentionROI.weight.sum())
                # ---------------------First resnet block-----------------------------------------------------------------------------------------------
                x = self.Net.bn1(x)
                x = self.Net.relu(x)
