#Clean training data generated by pointer net for Classification Evaluation and refinement net (Mark segments that in which the predicted segment does not match the GT segment (hence there is closer match between the Predicted segment and other Segment in the image_###########################################################################################################
...
#######################################Clean training data generated by pointer net for Classification Evaluation and refinement net (Mark segments that in which the predicted segment does not match the GT segment (hence there is closer match between the Predicted segment and other Segment in the image_###########################################################################################################
    def CleanerSeletor(self,PredDir,GTDir):
           ...
           for name in ListFile: # Get label image name
                ...
                if (GtAnn is None) or (PredAnn is None):
                         if os.path.isfile(GTDir + "/" + name): os.remove(GTDir + "/" + name)
                         if os.path.isfile(PredDir + "/" + name):os.remove(PredDir + "/" + name)
                         print("removed:"+name)
                         continue
                    
                if len(Ann.shape)==0: continue
